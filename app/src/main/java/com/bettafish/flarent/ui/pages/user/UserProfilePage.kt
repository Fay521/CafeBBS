...
fun UserProfilePage(
    ...
    val showTitle by remember {
        derivedStateOf {
            scrollState.value > (headerHeight - 10).coerceAtLeast(0) || headerHeight == 0
        }
    }
            ...
            Column(
                ...
                Column(modifier = Modifier.height(screenHeight)) {
                    ...
                    val nestedScrollConnection = remember {
                        object : NestedScrollConnection {
                            override fun onPreScroll(
                                ...
                                if (available.y.isNaN()) return Offset.Zero
                    ...
                    HorizontalPager(
                        ...
                        when (page) {
                            0 -> PagingDataList(posts, key = { it.id }) { post ->
                            ...
                            1 -> PagingDataList(discussions, key = { it.id }) { discussion ->
                                val owner = rememberViewModelStoreOwner(
                                    provider = storeProvider,
                                    key = discussion.id
                                )
