...
fun LoginPage(modifier: Modifier = Modifier,
    ...
    LaunchedEffect(Unit) {
        viewModel.loginSuccessEvent.collect { result ->
            App.INSTANCE.appSettings.userId = result.id
            App.INSTANCE.appSettings.token = result.token
            if (navigator != null) {
                // Started as root — navigate forward to HomePage
                navigator!!.navigate(HomePageDestination) {
                    launchSingleTop = true
                }
            } else {
                // Called from another page — go back with result
